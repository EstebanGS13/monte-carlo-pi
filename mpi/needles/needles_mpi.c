...
int main(int argc, char const *argv[]) {
    ...
    MPI_Reduce(&p_local, &p, 1, MPI_LONG_LONG_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        ...
        FILE *file = fopen("needles_mpi.csv", "a");
        if (file == NULL) {
            printf("No se puede abrir needles_mpi.csv");
            return -1;
        }
        fprintf(file, "%d, %d, %lld, %lld, %f, %f, %f, %f\n", num_processes, exponent, n, p, pi_approx, error, elapsed,
                wall_time);
        fclose(file);
    }
